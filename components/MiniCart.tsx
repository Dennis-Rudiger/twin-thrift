...
export default function MiniCart() {
  ...
  return (
    <div className="relative">
      ...
      {/* Drawer */}
      <div className={`${open ? 'pointer-events-auto opacity-100' : 'pointer-events-none opacity-0'} fixed inset-0 z-[60] transition-opacity`}
        aria-hidden={!open}
        onClick={() => setOpen(false)}
      />
      <aside
        className={`${open ? 'translate-x-0' : 'translate-x-full'} fixed right-0 top-0 z-[61] h-full w-full max-w-sm overflow-y-auto border-l border-lilac/30 bg-oat/40 p-4 backdrop-blur-xl transition-transform`}
        role="dialog"
      ...
      >
        ...
        {!hasItems ? (
          ...
          <div className="space-y-4">
            {cart.items.map((it) => (
              <div key={it.id} className="flex gap-3 rounded-xl border border-lilac/30 bg-oat/60 p-3">
